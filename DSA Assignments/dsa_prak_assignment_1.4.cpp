...
CityMatrix::CityMatrix(int matrix_size) { // Constructor with size input
...
CityMatrix::CityMatrix() { // Random Matrix Constructor
  srand(time(NULL));
  m_matrix_size =
      rand() %
...
CityMatrix::CityMatrix(const CityMatrix &other) {
...
CityMatrix &CityMatrix::operator=(const CityMatrix &other) {
  if (this == &other)
    return *this;
  // Clean up old memory
  delete[] m_best_path;
  ...
  void CityMatrix::setBestPath(int *path) {
    for (int i = 0; i < m_matrix_size; i++) {
      m_best_path[i] = path[i];
    }
  }

  void CityMatrix::printBestPath() {
    cout << "Best path is:";
    for (int i = 0; i < m_matrix_size; i++) {
      cout << " " << m_best_path[i];
    }
    ...
    Result CitySolver::solveGreedy(int startIndex) {
      ...
      for (int i = 0; i < n - 1; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
          ...
          if (!visited_cities[j] &&
              matrix->at(current_city, j) < smallest_cost) {
