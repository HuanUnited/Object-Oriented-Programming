...
void testConstructors() {
  ...
  for (size_t i = 0; i < bv2.size(); ++i) assert(bv2[i] == true);

  BitVector bv3(10, false);
  for (size_t i = 0; i < bv3.size(); ++i) assert(bv3[i] == false);
  ...
  for (size_t i = 0; i < bv5.size(); ++i) assert(bv5[i] == bv4[i]);

  // Assignment operator
  BitVector bv6;
  bv6 = bv4;
  assert(bv6.size() == bv4.size());
  for (size_t i = 0; i < bv6.size(); ++i) assert(bv6[i] == bv4[i]);
}
...
void testSetGetFlip() {
  ...
  bv.setAll(true);
  for (size_t i = 0; i < bv.size(); ++i) assert(bv[i] == true);

  bv.flipAll();
  for (size_t i = 0; i < bv.size(); ++i) assert(bv[i] == false);
